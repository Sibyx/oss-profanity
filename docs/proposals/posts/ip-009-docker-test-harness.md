...
| 2026-04-25 | jdubec | Superseded `/opt/baseline-eslint.config.mjs` location and `npm install -g` of all Node tools by IP-013. Toolchain now lives at `/opt/node-tools/` (unified ESLint + jscpd npm project); the `dockerfiles/eslint.config.mjs` file moved to `dockerfiles/node-tools/eslint.config.mjs` and a sibling `package.json` carries the pins. The Q8 committed-config principle is preserved; only the path and install shape changed. See [IP-013](ip-013-eslint-flat-config-fix.md). |
